 ...
 * `endpoint` is not a usable base URL. Everything that talks to a provider,
 * such as the agent proxy and the settings check, shares this, so "what
 * counts as a valid endpoint" is answered in exactly one place, and the
 * browser can ask the same question before it ever sends a request.
 *
