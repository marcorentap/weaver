...
export const askUserTool: PluginTool = {
  ...
  description: [
    ...
    "Nothing here waits for or continues with the answer: the run ends, the user answers the block whenever they like, and re-running inference on that block (or on anything below it) reads the answer as context.",
    "When a multichoice block appears in your context, it is prefixed with its label and reads as the question on its own line, then a `user:` line with the answer as bullets: `- option` for each picked option, `- Other: <text>` when Other was picked (with whatever the user typed into it), and `- Note: <text>` for the additional note. A block with no `user:` section is still unanswered — the question alone means the user has not picked anything yet, so read the options it lists as suggestions, not as the answer.",
